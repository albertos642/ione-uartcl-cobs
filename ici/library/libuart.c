/*
	libuart.c:	Common library for UART convergence-layer
			output daemon.
...
int	parseUartSpec(char *socketSpec,struct uartdescriptor *uart)
{
	...
	{
		writeMemoNote("[I] UART, Can not parse uart descriptor and \
baud rate!", socketSpecCopy);	
		return -1;
	}
...
//	writeMemoNote("[I] UART Baud Rate:", baudRate);	
		...
		{
        		writeMemoNote("[I] UART, Baud rate missing!",
					socketSpecCopy);	
        		return 0;
		}
	}

	writeMemoNote("[I] UART, Can not parse uart descriptor and/or \
baud rate!", socketSpecCopy);	
	return -1;
}
