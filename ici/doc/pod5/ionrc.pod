...
partition.  For this purpose it uses a set of default settings if no argument
...
shared working-memory partition.
...
The B<brief contacts> command.  This command writes a file of commands that
will recreate the current list of contacts, for the selected region, in the
ION database; the name of the file will be "contacts.I<region_number>.ionrc".
This file can be used to initialize the contact plan for a node that is newly
inserted into that region.

=item B<b passageway>

The B<brief passageways> command.  This command writes a file of commands
that will create registration IonContact objects in the node's ION database
identifying nodes that are passageways from the indicated region to other
regions.  The file's name will be "passageways.I<region_number>.ionrc".  This
file can be used to initialize the "rolodex" list for a node that is newly
inserted into that region.
