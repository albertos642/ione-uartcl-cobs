...
=item int ionPickRegion(uint32_t regionNbr)

Determines whether the indicated region is the node's home region, its
outer region (if any), or neither.  (For a brief discussion of the
"region" concept, see the irrrc(5) man page.)  Returns 0 if the
indicated region is the node's home region, 1 if it's the region's
outer region, -1 otherwise.

=item int ionRegionOf(uvast nodeNbrA, uvast nodeNbrB, uint32_t *regionNbr)

Determines which of the node's regions (home or, possibly, outer) is the
region in which nodes A and B both reside; if both nodes reside in both
regions, the local node's home region is determined to be the common
region; if I<nodeNbrB> is zero, the region in which node A resides is
determined to be the "common" region.  Places the region number of the
common region, if any, in I<*regionNbr> and returns 0 if the indicated
region is the node's home region, 1 if it's the region's outer region,
-1 otherwise.

=item int ionRemoteRegionOf(uvast nodeNbr, uint32_t *regionNbr)

Determines region in which the node identified by I<nodeNbr> resides
but the local node does not.  If there is such a region, it places
the number of that region in I<*regionNbr>; otherwise it places zero
in I<*regionNbr>. 
...
ionadmin(1), rfxclock(1), bpstats(1), llcv(3), lyst(3), memmgr(3), platform(3), psm(3), sdr(3), zco(3), ltp(3), bp(3), cfdp(3), ams(3), bss(3), irrrc(5)
