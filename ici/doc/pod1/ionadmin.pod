...
B<ionadmin> [ { I<commands_filename> | . | ! } [ I<vested_region_nbr> ] ]
...
If I<vested_region_nbr> is provided, then all commands pertaining to
contact plan configuration are processed even when the applicable region
number does not identify any region of which the node is a member.  This
enables "pre-registration", i.e., it pre-places contact plan information 
for the region in which the node is about to be (but has not yet been)
registered.
