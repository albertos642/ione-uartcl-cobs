/*
	owltsim6.c:	IPv6 enabled one-way light time delay simulator.
	Author: Scott M. Johnson, Spacely Packets, LLC

	Based on owltsim.c by Scott Burleigh

	Copyright (c) 2024, Spacely Packets, LLC
	ALL RIGHTS RESERVED.
...
/*	struct sockaddr		socketName;*/
	...
	if (buffer == NULL)
	{
		puts("owltsim6 out of memory.");
		owltsimExit(0);
	}

	/*	Create reception socket.				*/

	stp->insock = socket(AF_INET6, SOCK_DGRAM, IPPROTO_UDP);
	if (stp->insock < 0)
	{
	...
	if (stp->outsock < 0)
	{
		perror("owltsim6 can't open transmission socket");
		owltsimExit(1);
	}
