...
causes bsspclo to shut down. To restart bsspclo without rebooting ION
immediately, say, due to on-going data transfer over other outductss, one
can try to first use the bsspadmin 'x' command to stop bssp, then use the
bpadmin 'x' command to stop bp, then run ionrestart to repair any issue in
the non-volatile database, then use the bpadmin and bsspadmin 's' commands
to relaunch bundle protocol and its use of bssp.  This procedure could
temporarily enable resumption of operation until it is safe to reboot ION.
