...
commands directly into standard input.  If commands_filename is a period
(.), the effect is the same as if a command file containing the single
command 'x' were passed to tcaadmin -- that is, all of the TC's daemon
tasks are stopped.  If commands_filename is an exclamation point (!),
that effect is reversed: the TC's daemon tasks are restarted.
