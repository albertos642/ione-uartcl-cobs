/*
	libuartcla.c:	Common library for UART convergence-layer
			output daemon.
...
static int	openUartPortRead(int *uartPort,struct uartdescriptor *uartDes)
{
	...
	isprintf(mBuf, sizeof(mBuf), "[i] UART file descriptor:%s, length:%d",
			uartDes->uart_file_descriptor,
			strlen(uartDes->uart_file_descriptor));
	writeMemo(mBuf);
	writeErrmsgMemos();
  	*uartPort = open(uartDes->uart_file_descriptor, O_RDONLY);
	if (*uartPort < 0)
	{
		isprintf(mBuf, sizeof(mBuf), "[i] UART Error while opening \
device to read: %d  %s", errno, strerror(errno));
		writeMemo(mBuf);
		writeErrmsgMemos();
		return -1;
	}
	...
	{
		isprintf(mBuf, sizeof(mBuf),"[i] UART read Error %d from \
tcgetattr: %s", errno, strerror(errno));
		writeMemo(mBuf);
		writeErrmsgMemos();
		return -1;
  	}
	...
	switch (uartDes->baud_rate)
 	...
 	case 4800:
  	...
  	default:
		writeMemo("[i] warning: baud rate %u is not supported, \
using 115200");
		writeErrmsgMemos();
	...
	{
		isprintf(mBuf, sizeof(mBuf),"[i] UART Read Error from \
...
static int	openUartPortWrite(int *uartPort,struct uartdescriptor *uartDes)
{
	...
	writeErrmsgMemos();
	isprintf(mBuf, sizeof(mBuf), "[i] UartWriteSocket  file descriptor:%s, \
length:%d", uartDes->uart_file_descriptor,
			strlen(uartDes->uart_file_descriptor));
	writeMemo(mBuf);
	writeErrmsgMemos();
	...
	if (uart_port < 0)
	{
		isprintf(mBuf, sizeof(mBuf), "UART Error while opening device \
to write: %d  %s", errno, strerror(errno));
		writeMemo(mBuf);
		writeErrmsgMemos();
        	return -1;
	}
	...
	{
		isprintf(mBuf, sizeof(mBuf), "UART write Error %d from \
tcgetattr: %s", errno, strerror(errno));
		writeMemo(mBuf);
		writeErrmsgMemos();
        	return -1;
...
using 115200");
	...
	{
		isprintf(mBuf, sizeof(mBuf),"[i] UART Write Error from \
tcgetattr: %s\n", errno, strerror(errno));
		writeMemo(mBuf);
		writeErrmsgMemos();
	      	return -1;
		...
		struct uartdescriptor *socketName)
{
	...
	while (1)	/*	Continue until not interrupted.		*/
	{
		...
		isprintf(mBuf, sizeof(mBuf), "[i] UART, writing %d bytes to \
UART...", bytesWritten);
		writeMemo(mBuf);
		writeErrmsgMemos();
		if (bytesWritten < 0)
		...
		unsigned char *buffer)
{
	...
	{
		putErrmsg("Bundle is too big for UART CLA.",
				itoa(bundleLength));
		return -1;
	}
	...
	bytesToSend = zco_transmit(sdr, &reader, UARTCLA_BUFSZ,
			(char *) buffer);
	...
	bytesSent = sendBytesByUart(bundleSocket, (char *) buffer, bytesToSend,
			socketName);
	if (bytesSent < 0)
	{
		...
		if (*bundleSocket == -1)
		{
			...
			writeMemo("[i] Lost UART connection to CLI; restart \
CLO when connectivity is restored.");
			return 0;
		}
		else
		{
		...
		char *into, int length)
{
	...
	while (received < length)
	{
		...
		if (r < 0)
    		{
			writeMemo("[i] CLI read() error, failed to read from \
UART port");
			writeErrmsgMemos();      
			isprintf(mBuf, sizeof(mBuf),"[i] UART Error %d from \
read: %s", errno, strerror(errno));
			writeMemo(mBuf);
			writeErrmsgMemos();
			isprintf(mBuf, sizeof(mBuf), "[i] UART Port Nr: %d ",
					*uartPort);
			writeMemo(mBuf);
			writeErrmsgMemos();
			return -1;
			...
			{
				isprintf(mBuf, sizeof(mBuf), "[i] UART Induct \
