/*
	libuartcla.h:	Common library for UART convergence-layer
			output daemon.
...
extern int	sendBundleByUart(struct uartdescriptor *socketName,
			int *bundleSocket, unsigned int bundleLength,
			Object bundleZco, unsigned char *buffer);
extern int	receiveBytesByUart(int *bundleSocket, struct uartdescriptor
			*socketName, char *into, int length);
