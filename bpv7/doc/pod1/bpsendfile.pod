...
I<file_name>, then terminates.  When I<class_of_service> is omitted, the
bundle is sent at standard priority with no custody transfer requested;
for details of the I<class_of_service> parameter, see bptrace(1).
I<time_to_live>, if not specified, defaults to 300 seconds (5 minutes).
B<NOTE> that I<time_to_live> is specified B<AFTER> I<class_of_service>,
rather than before it as in bptrace(1).
