...
stcpclo6 - DTN simple TCP convergence layer adapter output task

=head1 SYNOPSIS

B<stcpclo6> I<remote_hostname>[:I<remote_port_nbr>]

=head1 DESCRIPTION

B<stcpclo6> is a background "daemon" task that connects to a remote node's
IPv6 TCP socket at I<remote_hostname> and I<remote_port_nbr>.  It then begins
...
B<stcpclo6> is spawned automatically by B<bpadmin> in response to the 's' (START)
command that starts operation of the Bundle Protocol, and it is terminated by
B<bpadmin> in response to an 'x' (STOP) command.  B<stcpclo6> can also be
...
B<stcpclo6> terminated normally, for reasons noted in the B<ion.log> file.  If
this termination was not commanded, investigate and solve the problem identified
in the log file and use B<bpadmin> to restart the STCP protocol.

=item "1"

B<stcpclo6> terminated abnormally, for reasons noted in the B<ion.log> file.
...
=item stcpclo6 can't attach to BP.
...
Redundant initiation of B<stcpclo6>.
