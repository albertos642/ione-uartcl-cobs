...
bundles destined for IMC-scheme (Interplanetary Multicast) endpoints and
determines which neighboring nodes those bundles should be sent to in order
to be forwarded to all members of the multicast groups identified by
those endpoints.  To do so, it reviews the preferred routes (computed
by Contact Graph Routing) to all group members, "gangs" the members by
route entry node (the initial node on each route), and forwards the
bundle once for each gang; each forwarded copy of the bundle is sent
to the entry node for the gang with an attached extension block listing
all gang members.
