...
sstcpcli6 - DTN simple TCP convergence layer input task

=head1 SYNOPSIS

B<stcpcli6> I<local_hostname>[!I<local_port_nbr>]

=head1 DESCRIPTION

B<stcpcli6> is a background "daemon" task comprising 1 + N threads: one that
...
The connection thread simply accepts connections on an IPv6  TCP socket bound to
...
B<stcpcli6> is spawned automatically by B<bpadmin> in response to the 's'
...
B<stcpcli6> terminated normally, for reasons noted in the B<ion.log> file.  If
this termination was not commanded, investigate and solve the problem identified
in the log file and use B<bpadmin> to restart B<stcpcli6>.

=item "1"

B<stcpcli6> terminated abnormally, for reasons noted in the B<ion.log> file.
Investigate and solve the problem identified in the log file, then use
B<bpadmin> to restart B<stcpcli6>.
...
Redundant initiation of B<stcpcli6>.
