...
tcpcli6 - DTN TCPCL-compliant convergence layer input task

=head1 SYNOPSIS

B<tcpcli6> I<local_hostname>[!I<local_port_nbr>]

=head1 DESCRIPTION

B<tcpcli6> is a background "daemon" task comprising 3 + 2*N threads: an
...
B<tcpcli6> records the acknowledgement flags, reactive fragmentation flag,
...
B<tcpcli6> is spawned automatically by B<bpadmin> in response to the 's'
...
response to an 'x' (STOP) command.  B<tcpcli6> can also be spawned and
...
B<tcpcli6> terminated normally, for reasons noted in the B<ion.log> file.  If
this termination was not commanded, investigate and solve the problem identified
in the log file and use B<bpadmin> to restart B<tcpcli6>.

=item "1"

B<tcpcli6> terminated abnormally, for reasons noted in the B<ion.log> file.
Investigate and solve the problem identified in the log file, then use
B<bpadmin> to restart B<tcpcli6>.
...
=item tcpcli6 can't attach to BP.
...
Redundant initiation of B<tcpcli6>.
...
=item tcpcli6 can't create access thread
