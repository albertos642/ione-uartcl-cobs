/*
	mailbundler.c:	emcapsulate batch mode emails into bundles.
	modified from below by Scott Johnson, Spacely Packets, LLC 2024
								*/
/*	Original file bpsource.c by								*/
...
[-t<Bundle TTL>]");
	...
	if (text)
	{
		...
		if (lineLength == 0)
		{
			putErrmsg("[?] No text for mailbundler to send.", NULL);
			bp_detach();
			return 0;
		}
		...
		if (bp_send(NULL, destEid, NULL, ttl, BP_STD_PRIORITY,
				...
				&newBundle) < 1)
		{
			putErrmsg("mailbundler can't send ADU.", NULL);
		}
	...
	while (_running(NULL))
	{
		printf(": ");
		fflush(stdout);
		igets(fd, line, sizeof line, &lineLength);
		if (lineLength == 0)
		{
                        line[0] = '-';
                        lineLength++;
...
#endif
	writeMemo("[i] Stopping mailbundler.");
	ionStopAttendant(_attendant(NULL));
	bp_detach();
	return 0;
}
