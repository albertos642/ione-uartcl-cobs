/*
	stcpclo6.c:	BP TCP-based convergence-layer output
	 ...
	 Author: Scott Johnson
        based on stcpclo.c by Scott Burleigh.
...
int	main(int argc, char *argv[])
{
	...
	if (ductName == NULL)
	{
		PUTS("Usage: stcpclo6 <remote host name>[!<port number>]");
		return 0;
	}

	if (bpAttach() < 0)
	{
		putErrmsg("stcpclo6 can't attach to BP.", NULL);
		return -1;
	}
	...
	if (buffer == NULL)
	{
		putErrmsg("No memory for TCP buffer in stcpclo6.", NULL);
		return -1;
	}
	...
	{
		putSysErrmsg("stcpclo6 can't create keepalive thread", NULL);
		MRELEASE(buffer);
	...
	while (!(sm_SemEnded(stcpcloSemaphore(NULL))))
	{
		...
		{
			writeMemo("[i] stcpclo6 outduct closed.");
			sm_SemEnd(stcpcloSemaphore(NULL));
			continue;
		}
	...
	pthread_mutex_destroy(&mutex);
	writeErrmsgMemos();
	writeMemo("[i] stcpclo6 duct has ended.");
	MRELEASE(buffer);
	ionDetach();
	return 0;
}
