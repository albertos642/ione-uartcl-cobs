/*
	libstcpcla6.c:	common functions for BP convergence-layer
			daemons that invoke simple TCP functionality.

        Author: Scott Johnson
        based on libstcpcla6.c by Scott Burleigh.
...
static int	connectToCLI6(char *protocolName, char *ductName, int *sock)
{
	if (*protocolName == '\0')
	{
	...
	switch (itcp_connect6(ductName, BpStcpDefaultPortNbr, sock))
	{
	case -1:
...
int	openStcpOutductSocket(char *protocolName, char *ductName, int *sock)
{
	...
	CHKERR(ductName);
	CHKERR(sock);
	return connectToCLI6(protocolName, ductName, sock);
}
		...
		Object bundleZco, char *buffer)
{
	...
	if (*sock < 0)
	{
		switch (connectToCLI6(protocolName, ductName, sock))
		{
		case -1:
