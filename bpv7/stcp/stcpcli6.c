/*
	stcpcli6.c:	BP TCP-based convergence-layer input
			daemon, designed to serve as an input
			duct.

 Author: Scott Johnson
        based on stcpcli.c by Scott Burleigh and Ted Piotrowski
...
int	main(int argc, char *argv[])
{
	...
	if (ductName == NULL)
	{
		PUTS("Usage: stcpcli6 <local host name>[:<port number>]");
		return 0;
	}

	if (bpAttach() < 0)
	{
		putErrmsg("stcpcli6 can't attach to BP.", NULL);
		return -1;
	}
	...
	{
		hostNbr.sin6_port = htons(BpStcpDefaultPortNbr);
	}

	
	
	atp.vduct = vduct;
	memset((char *) &(atp.socketName), 0, sizeof(struct sockaddr_in6));
	atp.inetName = (struct sockaddr_in6 *) &(atp.socketName);
	memcpy((char *) &(atp.inetName->sin6_addr), (char *) &hostNbr, 4);
